...
import { BarChart3, TrendingUp, Users, BookOpen, Download, Loader2 } from 'lucide-react'
...
function RelatoriosPage() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Usando o html-to-image, que suporta cores modernas (oklch) nativamente
      ...
      const pdf = new jsPDF('p', 'mm', 'a4') // Retrato, milímetros, folha A4
      
      const pdfWidth = pdf.internal.pageSize.getWidth()
      
      // Pega as propriedades reais da imagem gerada para calcular a altura
      const imgProps = pdf.getImageProperties(dataUrl)
      const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width
      
      pdf.addImage(dataUrl, 'PNG', 0, 10, pdfWidth, pdfHeight)
      pdf.save('relatorio-escola-discover.pdf')
      
    } catch (error: any) {
      console.error('Erro detalhado:', error)
      alert('O erro exato foi: ' + (error.message || error))
    } finally {
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 px-5 py-2.5 rounded-xl font-medium shadow-sm transition-all disabled:opacity-60"
        >
          {isExporting ? <Loader2 className="w-5 h-5 animate-spin text-gray-400" /> : <Download className="w-5 h-5 text-gray-500" />}
          {isExporting ? 'Gerando...' : 'Exportar PDF'}
        </button>
      </div>

      {/* A prop ref={relatorioRef} isola essa div para ir para o PDF */}
      <div ref={relatorioRef} className="space-y-6 bg-white p-2">
        
        {/* Cabeçalho exclusivo para o PDF (visível apenas na impressão) */}
        <div className="hidden print:block mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Relatório Consolidado - Discover</h2>
          <p className="text-gray-500">Métricas de engajamento do período letivo.</p>
        </div>
